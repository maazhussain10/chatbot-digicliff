...
exports.getColumnNames = async (username, assistantName, tableName) => {
  ...
  let DBconnection = await DataBaseConnection(hostname, dbUsername, password, databaseName);
...
exports.addQueryDetails = (
  ...
  for (let i = 0; i < selectedColumns.length; i++) {
    ...
    connection.query(
      sql,
      [username, assistantName, intentName, tableName, selectedColumns[i]],
      (err) => {
        if (err) console.log(err);
      }
    );
      ...
      if (err) console.log("Run Query:", err);
        ...
        if (type === "chip") {
          ...
          for (let i = 0; i < results.length; i++) {
            ...
            //Insert Chip
            createChip(username, assistantName, intentName, "false", chipValue);
          }
        } else if (type === "card") {
