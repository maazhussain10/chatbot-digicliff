        ...
        if (hasMultipleReply === "true") {
          for (let i = 0; i < responses.length; i++) {
            ...
            for (let j = 0; j < entities.length; j++) {
              if (
                message.slice(message.indexOf("$") + 1).split(" ")[0] ===
        ...
        if (!(queryCard.length === 0 && queryChip.length === 0)) {
          ...
          if (queryCard.length !== 0) {
            ...
            for (let i = 0; i < queryCard.length; i++) {
              ...
              for (let j = 0; j < queryCard[i].cardName.length; j++) {
                // Add columnName and value to card.
                values[queryCard[i].cardName[j]] = queryCard[i].cardValue[j];
              }
            }
        ...
        if (cards.length > 0 || chips.length > 0) {
          for (let i = 0; i < entities.length; i++) {
            ...
            if (cards.length != 0) {
              ...
              cards.map((card) => {
                let cardValue = JSON.stringify(card.cardValue).replace(
                  /"/g,
                  " %"
                );
                if (
                  cardValue.slice(cardValue.indexOf("$") + 1).split(" ")[0] ==
                  entities[i].entityName
                ) {
                  //   Replace the message containing $entityName with the entityValue we stored in the table above.
                  card.cardValue = JSON.parse(
                    cardValue
                      .replace(
                        cardValue.slice(cardValue.indexOf("$")).split(" ")[0],
                        entities[i].entityValue
                      )
                      .replace(/ %/g, '"')
                  );
        ...
        if (queryCard.length !== 0) {
          connection.query(
            ...
            (err) => {
              ...
              else console.log("Query Card RichResponses Deleted Successfully");
            }
          );
        } else if (queryChip.length !== 0) {
          connection.query(
            ...
            (err) => {
              if (err) console.log("Deletion Error", err);
              else console.log("Query Chip Responses Deleted Successfully");
            }
          );
        }
        res.send(data);
