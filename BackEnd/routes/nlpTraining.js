...
class NLP {
    ...
    nlp(app) {
        app.post('/nlp', async (req, res) => {
                ...
                if (intentName == "None") {
                    ...
                    if (hasMultipleReply === "true") {
                        for (let i = 0; i < responses.length; i++) {
                            ...
                            for (let j = 0; j < entities.length; j++) {
                                if (message.slice(message.indexOf("$") + 1).split(" ")[0] === entities[j].entity_name) {
                                    message = message.replace(message.slice(message.indexOf("$")).split(" ")[0], entities[j].entity_value)
                                }
                            }
                        ...
                        if (responses.length != 0) {
                            ...
                            for (let i = 0; i < entities.length; i++) {
                                if (message.slice(message.indexOf("$") + 1).split(" ")[0] === entities[i].entity_name) {
                                    message = message.replace(message.slice(message.indexOf("$")).split(" ")[0], entities[i].entity_value)
                                }
                            }
                            messages.push(message);
                        }
                    ...
                    if (cards.length > 0 || chips.length > 0) {
                        for (let i = 0; i < entities.length; i++) {
                            if (chips.length != 0) {
                                chips.map(chip => {
                                    if (chip.chip_value.slice(chip.chip_value.indexOf("$") + 1).split(" ")[0] == entities[i].entity_name) {
                                        chip.chip_value = chip.chip_value.replace(chip.chip_value.slice(chip.chip_value.indexOf("$")).split(" ")[0], entities[i].entity_value)
                                    }
                                })
                            }
                            if (cards.length != 0) {
                                cards[0].map(card => {
                                    if (card.card_value.slice(card.card_value.indexOf("$") + 1).split(" ")[0] == entities[i].entity_name) {
                                        card.card_value = card.card_value.replace(card.card_value.slice(card.card_value.indexOf("$")).split(" ")[0], entities[i].entity_value)
                                    }
                                })
                            }
                        }
                    ...
                    if (queryCard.length !== 0 || queryChip.length !== 0) {
                        connection.query('delete from richresponses where intent_id=? and use_query="false";', [intentId], (err) => {
            ...
            let intentName = await this.trainIntents(assistantId, hasFollowUp, previousIntent, sendMessage);
