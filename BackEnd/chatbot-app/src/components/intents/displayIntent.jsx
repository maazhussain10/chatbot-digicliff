...
class DisplayIntent extends Component {
    ...
    componentDidMount = () => {
        ...
        // console.log("ASDSA<",this.props.intent.previousIntent);
        // if (this.props.intent.previousIntent !=null) {
        //     console.log($('#card-body').addClass(' follow'));
        //     $('#card-body').addClass('follow');
        // }
    }
    //------------------------------------------------------------------DELETE INTENT AXIOS----------------------------------------------------------------------
    handleDeleteIntent = () => {
    ...
    render() {
        ...
        let { username } = JSON.parse(sessionStorage.getItem('userDetails'));
        let { assistantName } = JSON.parse(sessionStorage.getItem('assistantDetails'));
        const { intentName, intentDesc, previousIntent } = intent;
        ...
        return (
            <React.Fragment>
                {previousIntent ?
                    ...
                    <div className="card text-center style">
                        <a className="intentCard" onClick={() => this.handleIntent()} href={`/assistant/${username}/${assistantName}/${intentName}`}>
                            <div className="card-body" id="follow">
                                <div className="row">
                                    <h5 className="card-title col-md-3">{intentName}</h5>
                                    <p className="card-text col-md-7">{intentDesc} </p>
                                    <div className="col-md-2 d-flex justify-content-end">
