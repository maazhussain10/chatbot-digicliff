...
class CardService {
    ...
    async delete(intentId,cardValue, accessToken, setAccessToken) {
        let authInterceptor = createInterceptor(setAccessToken);
        let response = await authInterceptor.delete(API_URL + "chatbot/intent/card",{intentId,cardValue}, { headers: authHeader(accessToken) });
        return response;
    }
