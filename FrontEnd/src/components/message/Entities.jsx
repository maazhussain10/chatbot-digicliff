...
const Entity = (props) => {
  const handleChange = (e) => {
    const index = props.entities.findIndex(
      (entity) => entity.type === e.target.name
    );
      ...
      <div className="row container">
        <div className="col-md-1">
          <img
            ...
            src="https://img.icons8.com/ios-filled/20/4a90e2/info.png"
            aria-hidden="true"
            data-toggle="tooltip"
        ...
        <div className="col-md-5">
          ...
          >
            ...
            />
            <label className="custom-control-label" htmlFor={props.type}>
              {props.type}
            </label>
          </div>
        </div>
        ...
        <input
          ...
          name={props.type}
          className={
            'col-md-6 form-control ' + (props.selected ? '' : 'invisible')
          }
        />
      </div>
      <br />
...
const Entities = (props) => {
  ...
  useEffect(async () => {
    ...
    try {
      let response = await entityService.get(
        intentId,
        accessToken,
        setAccessToken
      );
      ...
      for (let i = 0; i < selectedEntities.length; i++) {
        const index = entities.findIndex(
          (entity) => entity.type === selectedEntities[i].type
        );
  ...
  const saveEntities = async (e) => {
    ...
    let selectedEntities = [...entities]
      ...
      .map((entity) => {
        return { type: entity.type, name: entity.name, order: entity.order };
      });
    try {
