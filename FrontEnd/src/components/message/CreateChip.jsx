...
const CreateChip = (props) => {
  ...
  const handleChange = (e) => {
    if (e.target.type === 'checkbox') setUseQuery(e.target.checked);
    else if (e.target.type === 'text') setChip(e.target.value);
  };
  ...
  const createChip = async (e) => {
    ...
    try {
      ...
      props.setChips([
        ...props.chips,
        { chipValue: chip, chipOrder: props.chips.length },
      ]);
      setChip('');
    } catch (e) {
      console.log(e);
  ...
  return (
    <React.Fragment>
      <button
        ...
        data-target="#createChip"
        disabled={props.disableChip}
      >
        Add chips
      </button>
      <div className="collapse mt-4" id="createChip">
        <form className="card card-body" onSubmit={createChip}>
          ...
          <div className="custom-control custom-switch">
            <input
              ...
              id="useQueryChip"
            />
            <label className="custom-control-label" htmlFor="useQueryChip">
              Use Query
            </label>
          </div>
        </form>
